...
typedef struct {
  uint32_t  (*Init)        (ARM_NAND_DEVICE *cfg);
  uint32_t  (*UnInit)      (ARM_NAND_DEVICE *cfg);
  uint32_t  (*PageRead)    (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg);
  uint32_t  (*PageWrite)   (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg);
  uint32_t  (*BlockErase)  (uint32_t row, ARM_NAND_DEVICE *cfg);
} const NAND_DRV;
