...
#include <MK70F15.H>                    /* MK70F15 Definitions                */
#include "NAND_MKxx.h"
 ...
 *---------------------------------------------------------------------------*/
static uint32_t Init         (ARM_NAND_DEVICE *cfg);
static uint32_t UnInit       (ARM_NAND_DEVICE *cfg);
static uint32_t PageRead     (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg);
static uint32_t PageWrite    (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg);
static uint32_t BlockErase   (uint32_t row, ARM_NAND_DEVICE *cfg);
 ...
 *  Return: (none)
 *----------------------------------------------------------------------------*/
static void SetupPageLay (ARM_NAND_DEVICE *cfg) {
  ARM_NAND_PAGE_LAYOUT *pgLay = cfg->page_layout;
  
  if (cfg->page_size == 2112) {
    /* Setup page layout */
    pgLay->spare.ofs_lsn = MK_POS_LSN;
    pgLay->spare.ofs_dcm  = MK_POS_COR;
    pgLay->spare.ofs_bbm  = MK_POS_BBM;
    pgLay->spare.ofs_ecc  = MK_POS_ECC;
    pgLay->sector_inc  = MK_SECT_INC;
    pgLay->spare_ofs = MK_SPARE_OFS;
    pgLay->spare_inc = MK_SPARE_INC;
  }
}
...
static uint32_t NandReset (void) {
  ...
  /* Set reset code */
  NFC->CMD2  = NFC_CMD2_BYTE1(ARM_NAND_CMD_RESET) | NFC_CMD2_CODE (NFC_CMD_RESET);
  NFC->CMD1  = NFC_CMD1_BYTE3(ARM_NAND_CMD_STATUS);
  ...
  for (i = NAND_TIMEOUT; i; i--) {
    if ((NFC->CMD2 & NFC_CMD2_BUSY_START_MASK) == 0) {
      if ((NFC->SR2 & ARM_NAND_STAT_FAIL) == 0) {
...
static uint32_t PageRead (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg) {
  ...
  NFC->CMD2 = NFC_CMD2_CODE  (NFC_CMD_READ)     |
              NFC_CMD2_BYTE1 (ARM_NAND_CMD_READ_1ST) ;
  NFC->CMD1 = NFC_CMD1_BYTE2 (ARM_NAND_CMD_READ_2ND) ;
  ...
  for (i = NAND_TIMEOUT; i; i--) {
    if ((NFC->CMD2 & NFC_CMD2_BUSY_START_MASK) == 0) {
      ...
      if (*ep == 0x81 || *ep == 0x00) {
        ...
        return (ARM_NAND_OK);             /* Page read successful               */
      }
      else {
        if (*ep & 0x80) {
          return (ARM_NAND_ECC_FAILED);
        }
        return (ARM_NAND_ECC_CORRECTED);
      }
    }
  }
  return (ARM_NAND_TIMEOUT);            /* Busy asserted or no response       */
}
...
static uint32_t PageWrite (uint32_t row, uint8_t *buf, ARM_NAND_DEVICE *cfg) {
  ...
  NFC->CMD2 = NFC_CMD2_CODE  (NFC_CMD_WRITE)    |
              NFC_CMD2_BYTE1 (ARM_NAND_CMD_PROGRAM_1ST) ;
  NFC->CMD1 = NFC_CMD1_BYTE2 (ARM_NAND_CMD_PROGRAM_2ND) |
              NFC_CMD1_BYTE3 (ARM_NAND_CMD_STATUS)  ;
  ...
  for (i = NAND_TIMEOUT; i; i--) {
    if ((NFC->CMD2 & NFC_CMD2_BUSY_START_MASK) == 0) {
      /* Check status */
      if (NFC->SR2 & ARM_NAND_STAT_FAIL) {
        return (ARM_NAND_PROGRAM_FAILED);         /* Program operation failed           */
      }
      return (ARM_NAND_OK);               /* Page write successful              */
    }
  }
  return (ARM_NAND_TIMEOUT);            /* Busy asserted or no response       */
}
...
static uint32_t BlockErase (uint32_t row, ARM_NAND_DEVICE *cfg) {
  ...
  NFC->CMD2 = NFC_CMD2_CODE  (NFC_CMD_ERASE)     |
              NFC_CMD2_BYTE1 (ARM_NAND_CMD_ERASE_1ST) ;
  NFC->CMD1 = NFC_CMD1_BYTE2 (ARM_NAND_CMD_ERASE_2ND) |
              NFC_CMD1_BYTE3 (ARM_NAND_CMD_STATUS)   ;
  ...
  for (i = NAND_TIMEOUT; i; i--) {
    if ((NFC->CMD2 & NFC_CMD2_BUSY_START_MASK) == 0) {
      if (NFC->SR2 & ARM_NAND_STAT_FAIL) {
        return (ARM_NAND_ERASE_FAILED);        /* Erase operation failed             */
      }
      return (ARM_NAND_OK);               /* Block erased successfuly           */
    }
  }
  return (ARM_NAND_TIMEOUT);            /* Busy asserted or no response       */
}
...
//  #endif
  ...
  NFC->SECSZ  = NFC_SECSZ_SIZE(cfg->page_size);
  #if (NAND_BUS_W == NAND_BUS_W16)
